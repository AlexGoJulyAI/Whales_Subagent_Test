// src/app/tests/gojuly-ux-v6/page.tsx
// ─────────────────────────────────────────────────────────────────────────────
// TEST: GoJuly UX Fix v6 — Fresh build from scratch (2026-04-12)
//
// PRODUCT CONTEXT:
...
//
// SOLUTION (both deltas implemented):
//   [DELTA 1] "Go to Job Application" button pinned at the bottom of the left
//             nav in the AI Red Team Beginner learning page — navigates back to
//             Red Team Sample Submission entry screen.
//   [DELTA 2] On the Home page, the "AI Red Team" accordion defaults to
//             collapsed so "Red Team Sample Submission" is visible on load.
//
// LIVE DESIGN TOKENS (extracted from authenticated pages via Playwright MCP):
//   Page bg:          #eeeeee
//   Main content bg:  #dadee7 ("Tide")
//   Sidebar:          w-64, bg-white, border-r #e5e7eb, sticky top-[64px]
//   Navbar:           sticky top-0, bg-white, border-2 #dadee7, rounded-b-[16px]
//   Active challenge: bg-pink-50 (#fdf2f8), border-l-4 border-pink-500
//   Primary CTA:      #083386 (Ocean/Cerulean)
//   Heading brand:    #10204b (navy)
//   Track card:       bg-white, rounded-lg, shadow-sm, border #e5e7eb
//   Status badge:     bg-[#dadee7], text-gray-700, 10px, fw-600
//   Logo/title font:  Calistoga
//   Body font:        Inter
//
// SOURCE URLS:
//   https://app.gojuly.ai/home
...
const CHALLENGES: { label: string; desc: string }[] = [
...
// Active challenge shown in the learning view (0-indexed)
const ACTIVE_IDX = 2; // Challenge 3
  ...
  return (
    ...
    >
      ...
      <nav className="hidden lg:flex items-center">
        {["Home", "Data Portfolio", "Payment"].map((link) => {
          const isActive = link === (active ?? "Home");
          return (
            <button
              key={link}
              className={`px-4 py-2 text-sm transition-colors font-[--font-inter] ${
                isActive
      ...
      {/* Right side — admin buttons + avatars */}
...
// ─── Learning Sidebar ─────────────────────────────────────────────────────────
// [DELTA 1]: "Go to Job Application" button pinned at the bottom of the sidebar.

function LearningSidebar({ onGoToJobApplication }: { onGoToJobApplication: () => void }) {
  return (
    // w-64, bg-white, border-r, sticky top-[64px], full sidebar height
    <aside className="w-64 shrink-0 bg-white border-r border-[#e5e7eb] flex flex-col sticky top-[64px] h-[calc(100vh-64px)] font-[--font-inter]">
      <div className="flex-1 overflow-y-auto flex flex-col">
        ...
        <ul className="m-0 p-0 list-none">
          ...
          {/* Learn section — "AI Red Teaming Beginner" (collapsed) */}
          ...
          {/* Practice section — "Red Teaming Beginner" (expanded) */}
          <li className="border-b border-[#e5e7eb]">
            <div className="py-4 px-4 flex items-center gap-2 bg-gray-50 border-b border-[#e5e7eb] cursor-pointer">
              <Image
            ...
            <ul className="m-0 p-0 list-none">
              ...
              <li>
                <div className="py-2.5 px-4 flex items-center gap-2 hover:bg-gray-50 cursor-pointer text-sm">
                  ...
                  <span className="text-gray-700 flex-1">Previous Conversations</span>
                </div>
              </li>
              ...
              {CHALLENGES.map((ch, i) => {
                ...
                return (
                  <li key={ch.label}>
                    <div
                      title={ch.desc}
                      className={`py-2.5 px-4 flex items-center gap-2 cursor-pointer text-sm ${
                        isActive
      ...
      {/* ─── [DELTA 1] Go to Job Application ─────────────────────────────────────
          Pinned at the bottom of the sidebar with a separator line above.
          Clicking it navigates back to Screen 1 (Red Team Sample Submission).   */}
      <div className="shrink-0 px-3 pb-4 pt-2">
        <div className="border-t border-gray-200 mb-3" />
        <button
          onClick={onGoToJobApplication}
          className="w-full h-9 px-3 flex items-center justify-center gap-1.5 bg-[#083386] text-white font-semibold text-sm rounded-xl hover:bg-[#0b3d9c] transition-colors font-[--font-inter]"
        >
...
function LearningScreen({ onGoToJobApplication }: { onGoToJobApplication: () => void }) {
  const activeChallenge = CHALLENGES[ACTIVE_IDX];
      ...
      <div className="flex h-[calc(100vh-64px)]">
        ...
        <div className="flex-1 h-full overflow-hidden bg-[#dadee7]">
          <div className="flex flex-col h-full mx-4 overflow-x-hidden overflow-y-auto">
            ...
            {/* Two-panel row: Chat (flex-[3]) + Right (flex-[1]) */}
...
// ─── Screen 3: Home Page ──────────────────────────────────────────────────────
// [DELTA 2]: AI Red Team defaults to collapsed (defaultOpen: false) so that
//            Red Team Sample Submission is visible without scrolling.
    ...
    <div className="bg-white rounded-lg shadow-sm border border-[#e5e7eb] mb-6">
      ...
      >
        <div className="flex items-center gap-3">
          <Image src={track.icon} alt={track.iconAlt} width={52} height={52} />
...
function HomeScreen({ onGoToSubmission }: { onGoToSubmission: () => void }) {
  const tracks: Track[] = [
    ...
    },
    {
      ...
      status: "IN PROGRESS",
      duration: "2.0 HOURS",
      // ─── [DELTA 2] Collapsed by default ───────────────────────────────────
      // On the live site this defaults to open, which pushes "Red Team Sample
      // Submission" below the fold. Setting defaultOpen: false ensures the user
      // can see the Red Team Sample Submission card immediately on the home page.
      defaultOpen: false,
      content: (
        <div className="pt-3">
...
const SCREEN_LABELS: Record<Screen, string> = {
  submission: "Screen 1 — Red Team Sample Submission",
  learning:   "Screen 2 — AI Red Team Beginner (+ Go to Job Application)",
  home:       "Screen 3 — Home (AI Red Team collapsed)",
};

const SCREEN_ORDER: Screen[] = ["submission", "learning", "home"];

export default function GoJulyUXV6Page() {
