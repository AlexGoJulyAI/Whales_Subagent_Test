...
## DESIGN RESEARCH — RUNS HERE, AFTER PHASE 1 IS CONFIRMED

**This section only executes after all P0 questions from Phase 1 above are confirmed.** Once the Question Status Tracker shows every P0 as `confirmed` (P1/P2 may be ★ CLIENT DEFERRED after two unanswered exchanges) — run Design Research now.

Execute the full Design Research phase defined in **v3_SKILL.md §Design Research** in this exact sequence:

1. Parse the client's confirmed responses for style keywords, product type, and surface type
2. Run the Playwright health check — if browser is closed, stop and ask client to restart it
3. Run the screenshot sweep (max 9 screenshots across named references, Mobbin, Cosmos/Dribbble)
4. Present all screenshots to the client — **HARD STOP** until the client picks exactly 3 anchor images
5. Run per-anchor extraction for each confirmed anchor: 6 zoom crops + canvas pixel sampling + live DOM extraction where available
6. Fill the Synthesis Table for each direction (A, B, C)

Do not proceed to **DESIGN RESEARCH HANDOFF** until all three Synthesis Tables are complete and the client has confirmed their 3 anchor picks.

---

## DESIGN RESEARCH HANDOFF

Before writing the brief, record the client's anchor selections so the Designer Agent can receive them. This section must be completed after the client confirms their 3 image picks (see v3_SKILL.md Step 2b). **If the anchor selection has not been completed, halt and run Design Research before proceeding.**

Add this block to `docs/research/DESIGN_RESEARCH.html` as the first visible section (before reference logs):

```
CLIENT IMAGE SELECTION
  Anchor A: [filename] — [one sentence: what it shows + why selected] → drives Direction A
  Anchor B: [filename] — [one sentence: what it shows + why selected] → drives Direction B
  Anchor C: [filename] — [one sentence: what it shows + why selected] → drives Direction C
  Status: CLIENT CONFIRMED — [date]
  All other screenshots: secondary reference only
```

Then include the same record verbatim in the PROJECT_BRIEF.md under a **## Design Research Anchors** section (between §7 and §8). This is the handoff mechanism — the Designer Agent reads this section to know which anchor image drives which direction.
