...
**Upstream input:** `docs/research/PROJECT_BRIEF.md` (produced by Intake Agent) + `docs/research/DESIGN_RESEARCH.html` (contains CLIENT IMAGE SELECTION record with Anchor A/B/C assignments + all anchor screenshots) + all client-uploaded assets.
**Downstream output:**
...
- `docs/research/MOCKUP_A.html`, `MOCKUP_B.html`, `MOCKUP_C.html` — one per approved journey; all three passed to Prototyper Agent. Client selects final direction after seeing all three interactive prototypes.
...
HANDOFF VALIDATION
  ...
  DESIGN_RESEARCH.html present and readable:    [yes / no — if no, halt and request from Intake Agent]
  CLIENT IMAGE SELECTION record present:        [yes / no — if no, halt. Cannot design without anchors]
  Anchor A filename confirmed:                  [filename or MISSING]
  Anchor B filename confirmed:                  [filename or MISSING]
  Anchor C filename confirmed:                  [filename or MISSING]
  All three anchor files exist on disk:         [yes / no — Glob("docs/design-references/*.png") to verify]
...
Produce **one reference block per direction** (A, B, C). Each direction's block must source rows 1 (Palette), 4 (Surface), and 5 (Signature moment) exclusively from its assigned anchor image. Rows 2, 3, and 6 may draw from the brief's Design Token Record or any anchor. No value may be invented — every hex, font, and radius must trace to an extracted source.

**First: read all anchor files and zoom crops before building any token block.**
```
Read("docs/design-references/<anchor-A-filename>")
Read("docs/design-references/<anchor-A-name>-zoom-bg.png")
Read("docs/design-references/<anchor-A-name>-zoom-card.png")
Read("docs/design-references/<anchor-A-name>-zoom-cta.png")
Read("docs/design-references/<anchor-A-name>-zoom-type.png")
Read("docs/design-references/<anchor-A-name>-zoom-detail.png")
Read("docs/design-references/<anchor-A-name>-zoom-nav.png")
```
Repeat for anchors B and C. Zoom crops are the primary extraction source — use canvas-sampled and zoom-estimated values from the Anchor Extraction Records in DESIGN_RESEARCH.html. If zoom crops do not exist (platform-only anchor), use the original anchor screenshot.

```
LIVING TOKEN REFERENCE — DIRECTION [A | B | C]
  Last updated: [Step N]
  Anchor image: <filename> — CLIENT CONFIRMED

  — PALETTE (from anchor [A/B/C] only) —
  Background:        [hex] — [url_extracted | canvas-sampled | zoom-estimated | screenshot-estimated] from <filename>
  Surface:           [hex] — [source] from <filename>
  Primary accent:    [hex] — [source] from <filename>
  Secondary accent:  [hex or none] — [source] from <filename>
  Text primary:      [hex] — [source] from <filename>
  Text secondary:    [hex] — [source] from <filename>
  Muted text:        [hex or not-visible] — [source] from <filename>
  Border/divider:    [hex or none] — [source] from <filename>
  Nav background:    [hex] — [source] from <filename>

  — TYPOGRAPHY (from brief §8 or any anchor) —
  Type: Display  — [family] [size]px/[lh] [weight] — letter-spacing [em] — source: [brief §8 | anchor file]
  Type: Heading  — [family] [size]px/[lh] [weight] — letter-spacing [em] — source: [brief §8 | anchor file]
  Type: Body     — [family] [size]px/[lh] [weight] — line-height [n] — source: [brief §8 | anchor file]
  Type: Caption  — [family] [size]px/[lh] [weight] — source: [brief §8 | anchor file]
  Type: Label    — [family] [size]px/[lh] [weight] — text-transform: [uppercase|none] — source: [brief §8 | anchor file]

  — SURFACE TREATMENT (from anchor [A/B/C] only) —
  Card radius:   [px] — [url_extracted | canvas-sampled | zoom-estimated] from <filename>
  Card shadow:   [box-shadow value] — [url_extracted | zoom-estimated] from <filename>
  Card border:   [none | Npx color] — [url_extracted | zoom-estimated] from <filename>
  Overlay/glass: [yes — describe | no] — zoom-estimated from <filename>

  — SPACING (from anchor [A/B/C] zoom crops) —
  Card internal padding: [tight <12px | medium 16–24px | generous 32px+] (~[px]) — zoom-estimated
  Item/card gap:         [tight | medium | generous] (~[px]) — zoom-estimated
  Section breathing:     [compressed | normal | airy] — zoom-estimated
  Content alignment:     [left-heavy | centered | right-aligned] — zoom-estimated

  — SHAPE (from anchor [A/B/C] zoom crops) —
  Button radius:   [none | small 4–8px | medium 12–16px | large 20–32px | pill] — [url_extracted | zoom-estimated]
  Input radius:    [same scale or not-visible] — [source]
  Avatar shape:    [circle | rounded | square | not-visible] — zoom-estimated

  — COMPONENT PATTERNS (from anchor [A/B/C] zoom crops) —
  Icon style:       [outlined | filled | rounded | sharp | not-visible]
  Image treatment:  [full-bleed | contained | rounded | circular | not-visible]
  Nav pattern:      [top-bar | bottom-tab | sidebar | floating]
  List pattern:     [cards | rows | tiles | full-width-sections]
  Active state:     [hex + treatment — from zoom-nav]

  — SIGNATURE MOMENT (from anchor [A/B/C] only) —
  [Exact description of the specific visual element borrowed from <filename> — name the component,
   its treatment, and what makes it distinctive. Must be traceable to a visible element in the screenshot.
   Name which zoom crop it appears in.]

  — COMPONENT TRANSLATION MAP (from anchor [A/B/C] zoom crops — REQUIRED before mockup generation) —
  [List 3–6 specific UI components visible in the anchor screenshot. For each, describe its exact structure
   and map it to the new product's equivalent screen. A mockup that uses only the anchor's color palette
   but ignores its component patterns is a FIDELITY VIOLATION — reject and regenerate before proceeding.]

  Component 1:
    Anchor component:   [name as seen in anchor — e.g. "hero metric with large numeral + label"]
    Anchor treatment:   [exact visual — layout, colors, shape, any visible text or icon]
    Anchor crop:        [which zoom crop or screenshot region]
    New-product target: [which screen / section of the new product]
    Translated as:      [how it appears in the mockup, preserving visual structure]

  [Repeat for each component — minimum 3, maximum 6. Every entry must trace to a visible element
   in the anchor screenshot. Generic descriptions ("rounded cards", "clean layout") are not acceptable.]

  — COMPONENTS (derived from all sections above) —
  Button (primary): bg [hex] | text [hex] [weight] [size] | radius [px] | height [px]
  [Repeat for every in-scope component]

  Active states:
    [ComponentName]: bg [hex] | text [weight + hex] | accent: [type] [color] [position]

  ★ CLIENT DEFERRED tokens applied:
    [Token name] | [Designer judgment applied] | [Rationale]
```

**Cross-direction check (run after all three blocks are built):**
- Do all three directions have meaningfully different Palette rows? If two share the same background and accent, one must be reconceived before proceeding.
- Does each Signature Moment describe a visually distinct element? Generic descriptions ("rounded cards", "clean layout") are not acceptable.
...
## DESIGN RESEARCH HANDOFF CHECK
*Design Research was completed by the Intake Agent (Phase 1). Before writing any mockup, verify the handoff is complete.*

**Hard gate — confirm all three before proceeding to OUTPUT 2:**
...
DESIGN RESEARCH HANDOFF CHECK
  ...
  Anchor B filename confirmed:                        [filename or MISSING]
  Anchor C filename confirmed:                        [filename or MISSING]
  All three anchor files exist on disk:               [yes / no — Glob("docs/design-references/*.png")]
  Per-direction Synthesis Tables present (A, B, C):   [yes / no — in DESIGN_RESEARCH.html]
  §Design Research Anchors in PROJECT_BRIEF.md:       [yes / no]
```

If any field is missing: escalate to the Intake Agent. Do not generate any mockup until all fields are confirmed.

The per-direction Synthesis Tables and Living Token Reference blocks (built from the Anchor Extraction Records in DESIGN_RESEARCH.html) are the design token source for all three mockups. Read them before writing any mockup file.
...
**Component fidelity gate — run before writing any mockup HTML:**
For each direction, open the Component Translation Map from its Living Token Reference Block (Step 5).
Verify that every mapped component appears in the mockup render. Use this checklist:
- [ ] Each anchor component is visible in the phone frame as a named element
- [ ] The component's visual structure (layout, color, shape) matches the translation spec
- [ ] No component entry is fulfilled by a generic card — each must be recognizably derived from the anchor

If any mapped component is absent, add it before moving to the next direction. A mockup that uses correct colors but missing component patterns fails this gate.
...
**Design principle: the right column serves client evaluation, not engineering handoff.** Every section must be scannable in under 10 seconds. Use key→value rows and short bullets — never prose paragraphs. Engineering detail (pixel redlines, handoff JSON, responsive breakpoint tables) belongs in the Prototyper's spec files (`MOCKUP_A.html`, `MOCKUP_B.html`, `MOCKUP_C.html`), not in the client-facing HTML.
